...
export default function ChatbotWidget() {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      addMessage("bot", "샐러드맨이에요! 무엇을 도와드릴까요?");
...
const fetchStoreByKeyword = async (keyword) => {
  ...
  const handleUserInput = (text) => {
    ...
    if (!mode) {
      if (cleaned.includes("메뉴")) {
        setMode("menu");
        setStep(1);
        addMessage("bot", "원하는 샐러드 종류가 있으신가요? (예: 비건, 소고기)");
  ...
  const handleMenuFlow = async (text) => {
    if (step === 1) {
      ...
      const menuList = result.map((menu) => `- ${menu.name} (${menu.salePrice.toLocaleString()}원)`).join("\n");
